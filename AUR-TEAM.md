[Home](Readme.md)  
[AUR Team on DreamWeave](https://github.com/orgs/DreamWeave-MP/teams/aur)  

# DreamWeave AUR Maintainership  

Dave Corley  
[AUR](https://aur.archlinux.org/account/S3ctor)  
[GitHub](https://github.com/magicaldave)  
[GitLab](https://gitlab.com/magicaldave1)  
[NexusMods](https://www.nexusmods.com/profile/S3ctorOMW)  
  
Settyness  
[AUR](https://aur.archlinux.org/account/settyness)  
[GitHub](https://github.com/Settyness)  
[GitLab](https://gitlab.com/Settyness)  
  
DreamWeave's AUR Packages:  
[MOMW Tools Pack](https://aur.archlinux.org/packages/momw-tools-pack)  
[s3lightfixes-git](https://aur.archlinux.org/packages/s3lightfixes-git)  
[vfstool-git](https://aur.archlinux.org/packages/vfstool-git)  
[delta-plugin-git](https://aur.archlinux.org/packages/delta-plugin-git)  
[groundcoverify-git](https://aur.archlinux.org/packages/groundcoverify-git)  
[umo-git](https://aur.archlinux.org/packages/umo-git)  
  
